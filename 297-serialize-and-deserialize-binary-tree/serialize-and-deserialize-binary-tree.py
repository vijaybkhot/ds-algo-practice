...
class Codec:
    def serialize(self, root):
        ...
        while q:
            for _ in range(len(q)):
                ...
                if node.left:
                    q.append(node.left)
                    serialized_tree += '#' + str(node.left.val) + "#"
                elif node.left is None:
                    serialized_tree += '#' + "^" + "#"
                if node.right:
                    q.append(node.right)
                    serialized_tree += '#' + str(node.right.val) + "#"
    ...
    def deserialize(self, data):
        ...
        q = deque()
        # Get root
        root = getNextNode(data)
        q.append(root)
        while q:
            for _ in range(len(q)):
                node = q.popleft()
                node.left = getNextNode(data)
                node.right = getNextNode(data)
